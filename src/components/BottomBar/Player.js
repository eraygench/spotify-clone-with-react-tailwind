...
import {setControls, setExpandedImage, setPlaying} from "stores/player";
...
function Player() {
    ...
    const {current, expandedImage} = useSelector(state => state.player)
        ...
        <div className="flex px-4 justify-between items-center h-full">
                        ...
                        <div className="flex items-center mr-3">
                            {!expandedImage &&
                                <div className="w-14 h-14 mr-3 relative flex-shrink-0 group">
                                    ...
                                    <button onClick={() => dispatch(setExpandedImage(true))} className="w-6 h-6 bg-black opacity-0 group-hover:opacity-60 hover:!opacity-80 hover:scale-[1.06] rounded-full absolute top-1 right-1 rotate-90 flex items-center justify-center">
            ...
            <div className="max-w-[45.125rem] w-[40%] pt-2 flex flex-col px-4 items-center">
                <div className="flex items-center gap-x-2">
                    ...
                    <button onClick={controls[state?.playing ? "pause" : "play"]} className="w-8 h-8 flex bg-white items-center justify-center text-black rounded-full hover:scale-[1.06]">
            ...
            <div className="min-w-[11.25rem] w-[30%] flex justify-end items-center">
                ...
                <div className="w-[8.813rem] max-w-full">
                    <CustomRange
                        step={0.01}
                        min={0}
                        max={1}
                        value={state?.muted ? 0 : state?.volume}
                        onChange={value => (state.muted && controls.unmute()) || controls.volume(value)}
                    />
                </div>
